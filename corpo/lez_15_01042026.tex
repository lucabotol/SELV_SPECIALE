...
Si trova ancora l'abete rosso subalpino.\\
...
\begin{itemize}[nosep]
    ...
    \item essendo elastico influenza la caduta della neve al suolo (nessuna protezione contro le valanghe);
...
È presente solamente nelle Alpi orientali, e le mughete sono protette nelle aree Natura 2000. Sono un problema nei pascoli abbandonati (regione esalpica), perché gli ha invasi: è un aspetto negativo se uno vuole tornare al pascolo.
\begin{figure}[htbp]
     \centering
     % Prima Immagine
     \begin{subfigure}[b]{0.25\textwidth}
...
Si trova nel piano subalpino (è la tipica specie pioniera di queste aree).\\
...
\begin{itemize}[nosep]
    ...
    \item adora i suoli fortemente minerali e con una forte dotazione di acqua;
    \item fortemente eliofila;
    \item ha bisogno di presenza di acqua, perché è un grande traspiratore (funziona come fosse una latifoglia);
    ...
    \item normalmente forma popolamenti pionieri. Tipicamente, nelle valli est-ovest colonizza il versante esposto a nord (i pini invece colonizzano i versanti esposti a sud);
    \item ha una pasciona rara, ogni 10-12 anni;
    \item tendenzialmente rifugge coperture erbose (in questi ambienti sono graminee, praterie);
    \item rispetto alla neve, non è un grande protettore dai suoi movimenti (poiché perde la chioma d'inverno e non interrompe il manto nevoso). Si comporta come se fosse una latifoglia;
